...
calculatedResults = zeros(1,100); % Calculated results initialization
    ...
    for p_index = 1:length(p_vals) % Run 100 times
        ...
        simulatedResults(p_index) = runTwoSeriesLinkSim(K, p, N); % Input the simulated results on the iteration
        calculatedResults(p_index) = K/(1 - p)^2; % Input the results from this equation on the iteration
    end
...
hold on; % Retains the above plots so that they don't get overwritten
% (Note: Simulated results on Y didn't make sense to be there)
semilogy(p_vals, calculatedResults, colorS(K_index), 'LineWidth', 1); % Plot for the calculated results through p_vals OOOO
